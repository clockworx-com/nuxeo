...
import static org.nuxeo.ecm.core.api.security.SecurityConstants.CAN_MAKE_RECORD;
import static org.nuxeo.ecm.core.api.security.SecurityConstants.CAN_MANAGE_LEGAL_HOLD;
import static org.nuxeo.ecm.core.api.security.SecurityConstants.CAN_SET_RETENTION;
...
public abstract class AbstractSession implements CoreSession, Serializable {
    ...
    public void makeRecord(DocumentRef docRef) {
        ...
        checkPermission(doc, CAN_MAKE_RECORD);
        DocumentModel docModel = readModel(doc);
    ...
    @Override
    public void setRetentionUntil(DocumentRef docRef, Calendar retainUntil) throws PropertyException {
        Document doc = resolveReference(docRef);
        if (!isRecord(docRef)) {
            throw new PropertyException("Document is not a record");
        }
        Calendar current = doc.getRetentionUntil();
        if (Objects.equals(current, retainUntil)) {
            // unchanged, don't do anything
            return;
        }
        checkPermission(doc, CAN_SET_RETENTION);
        Map<String, Serializable> options = new HashMap<>();
        options.put(CoreEventConstants.RETENTION_UNTIL, retainUntil);
        ...
        doc.setRetentionUntil(retainUntil);
        docModel = readModel(doc);
        notifyEvent(DocumentEventTypes.AFTER_SET_RETENTION, docModel, options, null, null, true, false);
    }

    @Override
    public Calendar getRetentionUntil(DocumentRef docRef) {
        Document doc = resolveReference(docRef);
        checkPermission(doc, READ);
        return doc.getRetentionUntil();
    }
    ...
    public void setLegalHold(DocumentRef docRef, boolean hold) {
        ...
        checkPermission(doc, CAN_MANAGE_LEGAL_HOLD);
        DocumentModel docModel = readModel(doc);
        Map<String, Serializable> options = new HashMap<>();
